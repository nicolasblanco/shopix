defmodule ShopixWeb.TranslationHelpersTest do
  ...
  test "root_path_unless_default_locale/2 returns the root path if passed locale is the default locale" do
    assert TranslationHelpers.root_path_unless_default_locale(
             %{assigns: %{global_config: %{default_locale: "en"}}},
             "en"
           ) == "/"
  end
  ...
  test "locales_filled_on/2 returns the locales which has been set" do
    ...
    assert TranslationHelpers.locales_filled_on(
             %{assigns: %{global_config: %{available_locales: ["en", "fr"]}}},
             data,
             :value
           ) == ["en"]
  end
  ...
  test "t/2 returns the translation if it exists in the conn" do
    conn =
      build_conn()
      |> assign(:translations, [
        %{key: "front.layout.title", value_translations: %{"en" => "foobar"}}
      ])
