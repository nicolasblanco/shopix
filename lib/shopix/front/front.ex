defmodule Shopix.Front do
  ...
  def create_or_add_order_with_product!(%Order{} = order, %Product{} = product, _),
    do: add_product_to_order!(order, product)

  def create_or_add_order_with_product!(nil, %Product{} = product, config),
    do: create_order_with_product!(product, config)
  ...
  def add_product_to_order!(%Order{} = order, %Product{} = product) do
    line_item =
      case Enum.find(order.line_items, fn li -> li.product_id == product.id end) do
        nil ->
          order
          |> Ecto.build_assoc(:line_items, product_id: product.id)
          |> Front.LineItem.changeset(%{})

        line_item ->
          LineItem.increase_quantity(line_item)
      end
