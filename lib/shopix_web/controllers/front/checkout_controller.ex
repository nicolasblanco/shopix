defmodule ShopixWeb.Front.CheckoutController do
  ...
  def address(conn, _params) do
    render(conn, "address.html",
      changeset: Front.change_order(conn.assigns.current_order),
  ...
  def payment(conn, _params) do
    render(conn, "payment.html",
      shipping_cost_amount:
        Order.shipping_cost_amount(
          conn.assigns.current_order,
          conn.assigns.global_config.shipping_cost_default_amount
        )
    )
  end
  ...
  def validate_payment(conn, %{"payment" => %{"nonce" => payment_nonce}}) do
    ...
    with {:ok, _} <- result,
      ...
      Email.order_complete_email(order, conn.assigns.current_locale, conn.assigns.global_config)
